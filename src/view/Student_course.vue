<template>
  <md-content class="md-scrollbar page-container md-layout-column" style="min-height:100vh">
    ...
    <Card  v-for="value in Scourse" 
           v-bind:name="value.course_name"
           info="3"></Card>
    </div>
...
export default {
  ...
  created:async function() {
      ...
      if(this.$store.state.Scourse ==""){
        ...
        data= await this.$dataSource.sGetCourse("17031803")
        if(data.status==0){
          clearInterval(delay)
          this.showToast=true
          this.Scourse=""
          ...
          clearInterval(delay)
          this.$store.commit("setScourse",JSON.stringify(data.info))
          this.Scourse=this.$store.state.Scourse
        }
      }
};
